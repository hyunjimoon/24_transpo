...
class TrafficGenerator:
    def __init__(self, max_steps, n_cars_generated, num_lanes, lane_length, speed_limit, left_turn):
        ...
        self._dir_name = f"intersection_flow{self._n_cars_generated}_lane{self._num_lanes}_length{self._lane_length}_speed{self._speed_limit}_left{self._left_turn}"
...
class TrafficGeneratorTransfer(TrafficGenerator):
    def __init__(self, source_path_name, max_steps, n_cars_generated, num_lanes, lane_length, speed_limit, left_turn):
    ...
    def generate_netfile(self, seed):
        ...
        if not os.path.exists(f"./results/{source_path_name}/transfer/network"):
            shutil.copytree(base_dir, f"./results/{source_path_name}/transfer/network")
        # print(f"Copied base directory to ./results/{dir_name}")

        
        # get input about lane length, lane numbers, and traffic light phases
        # net_raw = (open(str(Path(f"./results/{source_path_name}/transfer/{dir_name}/network/environment.net.xml")), 'r')
        net_raw = (open(str(Path(f"./results/{source_path_name}/transfer/network/environment.net.xml")), 'r')
            .read()
            .replace('13.89', str(self._speed_limit)))
        ...
        open(str(Path(f"./results/{source_path_name}/transfer/network/environment.net.xml")), 'w').write(net_raw)
    ...
    def generate_routefile(self, seed):
        ...
        with open(f"./results/{source_path_name}/transfer/network/episode_routes.rou.xml", "w") as routes:
            print("""<routes>
    <vType accel="1.0" decel="4.5" id="standard_car" length="5.0" minGap="2.5" maxSpeed="25" sigma="0.5" speedFactor="norm(1,0)"/>
